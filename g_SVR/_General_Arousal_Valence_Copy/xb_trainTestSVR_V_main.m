...
fTrainAV = fopen('za_Video_Train.txt','r'); %fTrainV = fopen('FinalVideoTrain.txt'); 
...
while ischar(tlineAV)
	c = c + 1;
    %tlineAV = 'VideoFeatures[VALorAROU]/P64/Video26600Feature.mat 0.11675 0.13674 0.18649 0.072348'; %Actually the Predicted Values from the file isnt used rather is taken from the .mat feature file
...
%     ratingV = str2double(fileInfoV(2)); 
    ...
    prdAV = fAV.predicted_value; %RAW PREDICTED VALUE
...
fTestAV = fopen('zb_Video_Test.txt'); %fTestV = fopen('FinalVideoTest.txt');
...
SMOOTH_RESULT_V = [mse.^0.5 q ccc]

predicted = predictedAV;
save('video.mat','actual','predicted')
...
RAW_RESULT_V = [mse.^0.5 q ccc]

predicted = predictedAV;
save('videoraw.mat','actual','predicted')
