...
fTrainAV = fopen('za_MFCC_Train.txt','r'); %fTrainV = fopen('FinalVideoTrain.txt'); 
...
while ischar(tlineAV)
    c = c + 1;
    %tlineAV = '%MFCC_Features/P23/MFCCFeatureAudio280.mat 0.11675 0.13674 0.18649 0.072348';
...
fTestAV = fopen('zb_MFCC_Test.txt'); %fTestV = fopen('FinalVideoTest.txt');
...
SMOOTH_RESULT_MFCC = [mse.^0.5 q ccc]

predicted = predictedAV;
save('mfcc.mat','actual','predicted')
...
RAW_RESULT_MFCC = [mse.^0.5 q ccc]

predicted = predictedAV;
save('mfccraw.mat','actual','predicted')
